...
// Problem 1
//runners full name and their company
let runnerCompany = [];
runners.forEach(function(comp) {
    runnerCompany.push(`${runner.first_name} ${runner.last_name} ${runner.company_name}`);
});
console.log(runnerCompany);

// Problem 2
//change emails to all caps
let emailCaps = runners.map(function(uppercase) {
    console.log(uppercase.email.toUpperCase());
});


// Problem 3
//all runners in company Wordtune

let wordtuneRunners = runners.filter(function(runner) {
    return runner.company_name === "Wordtune";
});
